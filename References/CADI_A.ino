...
public:
  ...
  MahonyAHRS(float kp = 2.0f, float ki = 0.005f)
      : q0(1.0f), q1(0.0f), q2(0.0f), q3(0.0f), integralFBx(0.0f),
        integralFBy(0.0f), integralFBz(0.0f), twoKp(2.0f * kp),
        twoKi(2.0f * ki) {}
...
public:
  ...
  PIDController(float p, float i, float d, float ff, float iMax)
      : kp(p), ki(i), kd(d), kff(ff), integralMax(iMax), integral(0),
        lastMeasured(0), lastDOut(0), firstRun(true) {}
...
SlewLimiter
    slewRoll(180.0f); // Relajado para permitir mayor maniobrabilidad táctica
...
void setup() {
  ...
  // Si se pierde un paquete, el siguiente con estado actualizado llegará en
  // 20ms. Esto elimina el jitter causado por el bloqueo del módulo durante
  // reintentos.
  ...
  xTaskCreatePinnedToCore(taskStabilization, "Stabilization", 4096, NULL,
                          5, // Alta prioridad para lazo PID 50Hz
                          NULL,
                          1 // Núcleo 1 (estabilización)
  );

  xTaskCreatePinnedToCore(taskNavigationComms, "NavigationComms", 8192, NULL,
...
void taskStabilization(void *pvParameters) {
  ...
  while (true) {
    ...
    if (imuActive) {
      ...
      float targetRollLocal, targetPitchLocal, targetYawLocal,
          targetThrottleLocal;
      ...
      // Mitigación de Torque Roll: rampa del acelerador para suavizar
      // aceleraciones
      static float currentThrottle = 0.0f;
      float throttleDelta = targetThrottleLocal - currentThrottle;
      float maxThrottleDelta =
          60.0f * dt; // max 60 unidades por segundo de rampa
      if (throttleDelta > maxThrottleDelta)
        currentThrottle += maxThrottleDelta;
      else if (throttleDelta < -maxThrottleDelta)
        currentThrottle -= maxThrottleDelta;
      else
        currentThrottle = targetThrottleLocal;
      ...
      servo4Pos =
          map(constrain(rollActuator, -45, 45), -45, 45, 135, 45); // invertido
      servo5Pos = 90;
      servo6Pos = 90;
...
void parseSetKeySerial() {
  while (Serial.available()) {
    ...
    if ((uint8_t)ch == MAGIC_CMD || (uint8_t)ch == MAGIC_TELEM ||
        (uint8_t)ch == MAGIC_WP)
      break;
    Serial.read();
    if (ch == '\n' || ch == '\r') {
  ...
  while (true) {
    ...
    if (currentMode == MODE_WAYPOINT || currentMode == MODE_ORBIT) {
      ...
      if (homeLat != 0.0f && homeLon != 0.0f) {
        ...
        if (hDist > 1800.0f) {
          // Si nos salimos de los 1800m de geocerca, forzar RTL automático
          // hacia home
          currentMode = MODE_WAYPOINT;
          waypointRoute[0].lat = homeLat;
          waypointRoute[0].lon = homeLon;
...
void estimateWind() {
  ...
  // Filtro exponencial de baja frecuencia (alpha = 0.05 — promedia ~20
  // muestras)
  windX = windX * 0.95f + rawWindX * 0.05f;
  windY = windY * 0.95f + rawWindY * 0.05f;
  windZ =
      windZ * 0.98f + 0.0f; // Viento vertical no estimable, mantener filtrado
}
...
void applySafetyGuards() {
  ...
  if (gforceLocal > G_SOFT_START) {
    float attenuation =
        1.0f -
        constrain((gforceLocal - G_SOFT_START) / (G_SOFT_END - G_SOFT_START),
                  0.0f,
                  0.7f // max 70% de reducción para no perder autoridad total
        );
    portENTER_CRITICAL(&sharedStateMutex);
...
void handleFhssHopping() {
  if (millis() - lastPacketReceivedMs > 1000) {
    // Si perdimos el enlace por más de 1 segundo, saltar de canal cada 100ms
    // buscando la base
    ...
    if (gps.location.isValid()) {
      ...
      if (firstGPSFix) {
        ...
        if (dt > 0.05f) {
          ...
          // Sanity check
          if (fabsf(gpsVx) < MAX_VEL_GPS && fabsf(gpsVy) < MAX_VEL_GPS &&
            ...
            kf3d.updateGPS(kf3d.posN() + dNorth, // posición Norte acumulada
...
void readMPU() {
  ...
  float axE = (1 - 2 * (q2 * q2 + q3 * q3)) * ax +
              2 * (q1 * q2 - q0 * q3) * ay + 2 * (q1 * q3 + q0 * q2) * az;
  float ayE = 2 * (q1 * q2 + q0 * q3) * ax +
              (1 - 2 * (q1 * q1 + q3 * q3)) * ay + 2 * (q2 * q3 - q0 * q1) * az;
  float azE = 2 * (q1 * q3 - q0 * q2) * ax + 2 * (q2 * q3 + q0 * q1) * ay +
              (1 - 2 * (q1 * q1 + q2 * q2)) * az;
...
void pushTelemetryAck() {
  ...
  secTelem.crc =
      calculateRadioCRC16((uint8_t *)&secTelem.telem, sizeof(TelemetryPacket));
